...
public class FruitEditController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		final String fruitToEdit = session.getNewFruitName();
		// reset to null, so if user closes the window, then this new fruit
		// won't be in session
		session.setNewFruitName(null);
		...
		boxTypes.addAll(boxDao.getBoxSizesForFruit(fruitToEdit));
		qualTypes.addAll(boxDao.getQualityTypesForFruit(fruitToEdit));
		...
		btnSave.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				boxDao.deleteFruitDetails(fruitToEdit);
				//System.out.println(boxTypes);
				//System.out.println(qualTypes);
				boxDao.addFruitBoxSizes(fruitToEdit, listBoxes);
				boxDao.addFruitQualities(fruitToEdit, listQualities);
