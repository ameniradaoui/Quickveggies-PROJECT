...
public class FreshEntryController implements Initializable {
	...
	private static BuyerDao bd = BeanUtils.getBean(BuyerDao.class);
	...
	private static List<ExpenseInfo> expenseList = bd.getBuyerExpenseInfoList();
	...
	public void initialize(final URL location, final ResourceBundle resources) {
		...
		// if ((grNo.getText().isEmpty() ||
		// grNo.getText().trim().isEmpty())||(expenses.getText().isEmpty() ||
		// expenses.getText().trim().isEmpty())||(grower.getText().isEmpty()||
		// grower.getText().trim().isEmpty())
		// ||(truckNo.getText().isEmpty()||
		// truckNo.getText().trim().isEmpty())||
		// (driverNoField.getText().isEmpty()
		// ||driverNoField.getText().trim().isEmpty())||
		// (chlNo.getText().isEmpty() || chlNo.getText().trim().isEmpty())
		//
		// || (fwAgent.getText().isEmpty() ||
		// fwAgent.getText().trim().isEmpty())
		// || (totalQty2.getText().isEmpty()||
		// totalQty2.getText().trim().isEmpty())
		// || (halfCase.getText().isEmpty() ||
		// halfCase.getText().trim().isEmpty())|| (remarks.getText().isEmpty()
		...
		ChangeListener<Boolean> dateFocusChangeListener = (ObservableValue<? extends Boolean> observable,
		...
		sessionController.linkController(this);
		// SUPPLIER ID SECTION
		// set fruit selection:
		List<String> frNames = boxDao.getAllFruitTypes();
		...
		fwAgent.linkToWindow(FreshEntryController.this, "/fxml/supplieradd.fxml", "Add new supplier", STR_ADD_NEW,
				new AddSupplierController());
		fwAgent.focusedProperty().addListener(
		 ...
		 * quantity column in the edit mode state, so this key event handler is
		 * being added.
		 */
		...
		tcBuyerSelect.setCellValueFactory(new PropertyValueFactory<>("buyerSelect"));
		tcBuyerSelect.setCellFactory(
				new Callback<TableColumn<FreshEntryTableData.BuyerEntryTableLine, String>, TableCell<FreshEntryTableData.BuyerEntryTableLine, String>>() {
					...
					public TableCell<FreshEntryTableData.BuyerEntryTableLine, String> call(
						...
						final AutoCompleteTableCell autCompCellBuyerSelect = new AutoCompleteTableCell(acTextField);
						...
						autCompCellBuyerSelect.setOnKeyReleased((KeyEvent event) -> {
							if (event.getCode() == KeyCode.ENTER) {
								...
								if (selectedBuyer == null || selectedBuyer.trim().isEmpty()
										|| !buyersList.contains(selectedBuyer)) {
								...
								buyersEntry.getFocusModel().focus(row,
										(TableColumn<BuyerEntryTableLine, String>) nextTabColumnToFocus);
		...
		expenses.setOnMouseClicked((MouseEvent event) -> {
			try {
				...
				for (String name : charges1.keySet()) {
					sb.append(name + " rate:").append(charges1.get(name).totalValue.getText().toString())
							.append("\n * ");
				}
		...
		saveButton.setOnMousePressed((Event event) -> {
			...
			if (checkQuantities() == 0) {
				...
				client.saveEntryToSql("arrival",
						new String[] { "date", "fruit", "challan", "supplier", "totalQuantity", "fullCase", "halfCase",
								"fwagent", "truck", "driver", "gross", "charges", "net", "remarks", "dealID", "type",
								"amanat" },
						new String[] { date.getValue().toString(), fruitSelection.getValue(), chlNo.getText(),
								grower.getText(), totalQty2.getText(), fullCase.getText(), halfCase.getText(),
								fwAgent.getText(), truckNo.getText(), driverNoField.getText(), gross.getText(),
								charges.getText(), net.getText(), remarks.getText(), dealID + "",
								freshEntryTypes[freshEntryType], amanat });
				// BUYER DEALS PART:
				// Clean deals first
				client.deleteTableEntries("buyerDeals", "dealId", String.valueOf(dealID), false, false);
				for (String[] buyerSqlLine : FreshEntryTableData.buyerEntryLinesSql) {
					...
					client.saveEntryToSql("buyerDeals",
							new String[] { "buyerTitle", "dealDate", "buyerRate", "buyerPay", "boxes",
									"aggregatedAmount", "dealID", "boxSizeType", "qualityType", "fruit" },
							new String[] { buyerSqlLine[0], date.getValue().toString(), bRate, buyerSqlLine[6], boxes,
									aggregateAmt, dealID + "", buyerSqlLine[4], buyerSqlLine[3], buyerSqlLine[7] });
				}
				sessionController.resetPendingLadaanEntries();
				// SUPPLIER DEALS PART:
				// Clean deals first
				client.deleteTableEntries("supplierDeals", "dealId", String.valueOf(dealID), false, false);
				for (String[] supplierSqlLine : FreshEntryTableData.growerEntryLinesSql) {
					client.saveEntryToSql("supplierDeals",
							new String[] { "supplierTitle", "date", "cases", "supplierRate", "net", "agent", "dealID",
									"boxSizeType", "qualityType", "fruit" },
				...
				if (sessionController.getCharges() != null) {
					cd.addDealCharges(sessionController.getCharges(), dealID);
				}
	...
	public void keyReleasedProperty() {
		boolean isDisabled = (grNo.getText().isEmpty() || grNo.getText().trim().isEmpty())
				|| (expenses.getText().isEmpty() || expenses.getText().trim().isEmpty())
				|| (grower.getText().isEmpty() || grower.getText().trim().isEmpty())
				|| (truckNo.getText().isEmpty() || truckNo.getText().trim().isEmpty())
				|| (driverNoField.getText().isEmpty() || driverNoField.getText().trim().isEmpty())
				|| (chlNo.getText().isEmpty() || chlNo.getText().trim().isEmpty())

				|| (fwAgent.getText().isEmpty() || fwAgent.getText().trim().isEmpty())
				|| (totalQty2.getText().isEmpty() || totalQty2.getText().trim().isEmpty())
				|| (halfCase.getText().isEmpty() || halfCase.getText().trim().isEmpty())
				|| (remarks.getText().isEmpty() || remarks.getText().trim().isEmpty());
		...
		return (new EventHandler<TableColumn.CellEditEvent>() {
			...
			public void handle(TableColumn.CellEditEvent event) {
				...
				if ((currColNum + 1) < totalCols) {
					...
					System.out.println("The newly focused cell is of column: " + nextColInSeq.getText()); // Todo
																											// debug
...
//			 //"Can't continue-current table hasn't been properly filled!";
//			 if(currTable == LOT) { addLots.requestFocus();
...
//			  GeneralMethods.errorMsg(emptyFieldMsg);
//				else { // if the entry is a regular one, continue to populate the // buyers table 
//		if(freshEntryType == FreshEntryController.REGULAR) {
//			  buyersEntry.setEditable(true);
//			  buyersEntry.requestFocus();
//			  currTable = BUYERS; } // if the entry is coldstore/godown, skip the buyers and // check the growers else { // check if more
//			  //grower lines are needed 
//			  if (totalGrowersQty <Integer.parseInt(addLotLines.get(addLotLines.size() - 1).getQty())) {
//				  addTableLine(growerLines,data.getGrowerEntryTableLine("", "0", "0", ""));
//			     growersEntry.requestFocus(); currTable = GROWERS; } else {
//			    	 //prepare grower entries for sql, such that are
//			    	 // specific foreach deal // variables: box size,quality,rate 
//			    	 for
...
//			  updateTable(growersEntry, growerLines);
//			  growersEntry.setEditable(false); // check if more lot lines are needed 
//			  if (totalLotQty < Integer.parseInt(totalQty.getText())) {
//			  addTableLine(addLotLines, data.getAddLotTableLine("", "0", ""));
//			  // addLots.setEditable(true); addLots.requestFocus(); currTable =LOT; 
//			  } }}}}
//            else if(currTable==BUYERS){
//            	sessionController.setSupplierGrossSum(sessionController.getSupplierGrossSum()+getSuppliersGross());gross.setText(sessionController.getSupplierGrossSum()+"");
//            	updateAmountCalc();
//            	// calculate
//			(areTableFieldsEmpty(buyersEntry))GeneralMethods.errorMsg(emptyFieldMsg);else{checkQuantities();
//	// check if more buyers lines are needed 
//			if (totalBuyersQty <Integer.parseInt(growerLines.get(growerLines.size()-1).getGrowerQty())){addTableLine(buyerLines,data.getBuyerEntryTableLine("","0","0",""));}
//			else{ //
//
//	prepare buyers entries for
//	sql // variables: //
...
//			 Integer.parseInt(line.getBuyerQty())) }); }buyerLines.clear();buyerLines.add(data.getBuyerEntryTableLine("","0","0","")); //
//	data.disableCommittedLines(growerLines);
//
//	updateTable(buyersEntry,
...
//	}}}}
	 ...
	 * @param comboBox
	 * @param tc
	 *            - optional table column to move the focus to, set to null if
	...
	private void setTableComboKeyPressListener(final TableView<?> tv, final ComboBox<?> comboBox,
		...
		comboBox.setOnKeyPressed((KeyEvent event) -> {
			if (event.getCode() == KeyCode.ENTER) {
				/*
				 * System.out. println(
				 * "in table combo box enter key event, for table column: " +
				 * tc.getText());
				 */
				...
				// + currRow);
				tv.getSelectionModel().clearAndSelect(currRow);// Todo: fix: ,
																// tc
	...
	private int handleLastColOfBuyerTable(TableView<FreshEntryTableData.BuyerEntryTableLine> tv, int curRow,
		...
		if (buyerQty < totGrowerQtyForBoxType) {
			 ...
			 * table, now switch to growers table if there are more entries to
			 * be made
			 */
			...
			if (totGrowerQtyForFruitQuality < qtyOfLotForQuality) {
				...
				startNextColumnEdit(growersEntry, grListLastRow,
						(TableColumn<GrowerEntryTableLine, String>) nextTabColumnToFocus);
				growersEntry.requestFocus();
				return -1;
			}
	...
	private void addBuyerDataToQueryList() {
		for (FreshEntryTableData.BuyerEntryTableLine line : buyerLines) {
			...
			FreshEntryTableData.buyerEntryLinesSql.add(new String[] { line.getBuyerSelect(), grower.getText(),
					...
					"" + (Integer.parseInt(line.getBuyerRate()) * Integer.parseInt(line.getBuyerQty())),
					fruitSelection.getValue() });
		}
	}
	...
	private void addGrowerDataToQueryList() {
		recalculate();
		for (FreshEntryTableData.GrowerEntryTableLine line : growerLines) {
			FreshEntryTableData.growerEntryLinesSql.add(
					new String[] { line.getGrowerRate(), line.getBoxSize(), line.getQuality(), line.getGrowerQty() });
		}
	}
	...
	private void resetFruitQualsAndBoxes() {
		...
		List<BoxSize> boxSizes = boxDao.getBoxSizesForFruit(fruit);
		List<QualityType> qualityTypes = boxDao.getQualityTypesForFruit(fruit);
