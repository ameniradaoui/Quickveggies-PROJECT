...
import com.quickveggies.dao.AccountEntryPaymentDao;
...
public class DBankAccountsController implements Initializable {
	...
	private static Long generatedKey = null;
	...
	private static ExpenditureDao ed = BeanUtils.getBean(ExpenditureDao.class);

	private static AccountEntryPaymentDao apd = BeanUtils.getBean(AccountEntryPaymentDao.class);

	private AccountEntryLineDao accountEntryDao = BeanUtils.getBean(AccountEntryLineDao.class);

	private MoneyPaidRecdDao md = BeanUtils.getBean(MoneyPaidRecdDao.class);
	...
	static {
		for (String expendType : ed.getExpenditureTypeList()) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		apd = BeanUtils.getBean(AccountEntryPaymentDao.class);
		accountEntryDao = BeanUtils.getBean(AccountEntryLineDao.class);
		...
		for (final AccountBox box : accountBoxesList) {
			...
			payeeNameCol.setCellFactory((final TableColumn<AccountEntryLine, String> param) -> {
				...
				field.setOnKeyReleased((KeyEvent t) -> {
					if (t.getCode() == KeyCode.BACK_SPACE) {
						...
						dbclient.updateTableEntry("accountEntries", line.getId(), new String[] { "payee" },
								new String[] { "" }, false, "");
					}
				});
				return cell;
			...
			expenseCol.setCellFactory(
					new Callback<TableColumn<AccountEntryLine, String>, TableCell<AccountEntryLine, String>>() {
						...
						public TableCell<AccountEntryLine, String> call(TableColumn<AccountEntryLine, String> param) {
							...
							comboBoxCell.setComboBoxHandler(
							...
							comboBoxCell.getComboBox().focusedProperty()
					...
					new Callback<TableColumn<AccountEntryLine, String>, TableCell<AccountEntryLine, String>>() {
						...
						public TableCell<AccountEntryLine, String> call(TableColumn<AccountEntryLine, String> param) {
							...
							button.setOnMouseClicked((MouseEvent event) -> {
								...
								// entry line INSIDE the array of lines for this
								// table
							...
							button.setOnMouseClicked(new EventHandler<MouseEvent>() {
								...
								public void handle(MouseEvent event) {
									...
									dbclient.updateTableEntry("accountEntries", line.getId(), new String[] { "status" },
											new String[] { line.getStatus() + "" }, false, "Excluded bank acc entry");
									populateAccountTransactionsTable(selectedAccountBox);
								}
							});
							return cell;
							...
							button.setOnMouseClicked((MouseEvent event) -> {
								...
								if (line.getDepositCol() == 0 && line.getWithdrawalCol() == 0) {
									dbclient.updateTableEntry("accountEntries", line.getId(),
											...
											new String[] { line.getPayee(), "0", "0" }, false);
								}
								// repopulate the newTransactionsTable
								populateAccountTransactionsTable(selectedAccountBox);
							});
			...
			delete.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					if (selected.isSelected()) {
						// if this account is currently displaying its tables,
						// clear them
						tablesDisappear();
					}
					try {
						...
						ok.setOnMouseClicked(new EventHandler<MouseEvent>() {
							...
							public void handle(MouseEvent event) {
								...
								dbclient.deleteTableEntries("accountEntries", "accountName", box.getAccountName(),
										true);
								refresh.fire();
						...
						// --------------------
						try {
							if (accountEntryDao.getAccountEntryLines(box.getAccountName()).size() > 0) {
			...
			edit.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					try {
						...
						Account accountToEdit = accountEntryDao.getAccountByName(box.getAccountName());
	...
	private void updateTabTables(TableView table, int status, Label noEntriesLabel) {
		...
		if (status == AccountEntryLine.INSOFTWARE || status == AccountEntryLine.EXCLUDED) {
					...
					new Callback<TableColumn<AccountEntryLine, Integer>, TableCell<AccountEntryLine, Integer>>() {
						...
						public TableCell<AccountEntryLine, Integer> call(TableColumn<AccountEntryLine, Integer> param) {
							...
							cell.setOnButtonPress(() -> {
								// Move the entity from inSoftware or Excluded
								// tab to newTransactions tab
								AccountEntryLine line = (AccountEntryLine) cell.getTableRow().getItem();
								Long id = line.getId();
								if (status == AccountEntryLine.INSOFTWARE) {
									// Move the entity from inSoftware or
									// Excluded tab to newTransactions tab
									accountEntryDao.setAccountEntryStatus(id, 0,
											"Removed InSoftware Entry from system");
								} else {
									accountEntryDao.setAccountEntryStatus(id, 0, "Removed Excluded Entry from system");
								}
								try {
									AccountEntryLine parent = accountEntryDao.getAccountEntryLine(line.getParentId());
									if (line.getWithdrawalCol() > 0.) {
										double leftAmount = parent.getWithdrawalCol() + line.getWithdrawalCol();
										dbclient.updateTableEntry("accountEntries", line.getParentId(), "withdrawalCol",
												String.valueOf(leftAmount), "");
									} else {
										double leftAmount = parent.getDepositCol() + line.getDepositCol();
										dbclient.updateTableEntry("accountEntries", line.getParentId(), "depositCol",
												String.valueOf(leftAmount), "");
									}
									accountEntryDao.deleteAccountEntry(id);
									List<AccountEntryPayment> payments = apd.getAccountEntryPayments(id);
									if (!payments.isEmpty()) {
										AccountEntryPayment payment = payments.get(0);
										if ("partyMoney".equalsIgnoreCase(payment.getPaymentTable())) {
											md.deleteMoneyPaidRecd(payment.getPaymentId());
										} else if ("expenditures".equalsIgnoreCase(payment.getPaymentTable())) {
											ed.deleteExpenditureEntry(payment.getPaymentId(), true);
										}
									}
									apd.deleteAccountPaymentByEntryId(id);
									accountEntryDao.setAccountEntryStatus(parent.getId(), 0, "");
								} catch (SQLException | NoSuchElementException ex) {
									Logger.getLogger(DBankAccountsController.class.getName()).log(Level.SEVERE, null,
											ex);
								}
								if (status == AccountEntryLine.INSOFTWARE) {
									populateAccountTransactionsTable(selectedAccountBox);
									updateTabTables(inSoftwareTable, AccountEntryLine.INSOFTWARE,
											noEntriesInSoftwareLabel);
								} else {
									populateAccountTransactionsTable(selectedAccountBox);
									updateTabTables(excludedTable, AccountEntryLine.EXCLUDED, noEntriesExcludedLabel);
								}
		...
		List<AccountEntryLine> accLines = new ArrayList<>();
		try {
			accLines = accountEntryDao.getAccountEntryLines(selectedAccountBox.getAccountName());
		...
		if (accountsNum > 0) {
			...
			for (int i = 0; i < accountsNum; i++) {
				...
				try {
					...
					accountBox.linkBankAccount(accountEntryDao.getAccountById(i + 1l));
	...
	private void populateAccountTransactionsTable(AccountBox box) {
		...
		// List<Account> accounts = acc
		List<AccountEntryLine> accEntryLines = new ArrayList<>();
		List<AccountEntryLine> newTransEntryLines = new ArrayList<>();
		try {
			accEntryLines = accountEntryDao.getAccountEntryLines(box.getAccountName());
		} catch (SQLException e) {
			e.printStackTrace();
			return;
		...
		Account account;
		try {
			account = accountEntryDao.getAccountByName(box.getAccountName());
		} catch (SQLException e) {
			e.printStackTrace();
			return;
		}
		double balance = 0;
		if (accEntryLines.size() > 0) {
			AccountEntryLine entry = accEntryLines.get(0);
			balance = entry.getBalanceCol();
		...
		ObservableList<AccountEntryLine> observableList = FXCollections.observableList(newTransEntryLines);
		newTransactionsTable.setItems(observableList);
	...
	public static Stage buildWindowForExpense(AccountEntryLine line, EntityType eType, String bankName) {
		...
		case MONEY_RECEIVED:
			controller = new MoneyPaidRecdController(eType, AmountType.RECEIVED, false,
					Double.toString(line.getDepositCol()), PaymentMethodSource.Bank, line.getPayee(), line.getDateCol(),
					line.getChqnoCol(), bankName);
		...
		default:
			controller = new ExpenseAddController(Double.toString(line.getWithdrawalCol()), line.getPayee(), expense,
					line.getPayeeType(), line.getDateCol(), line.getChqnoCol(), line.getAccountName(),
		...
		Long key = ((DaoGeneratedKey) controller).getGeneratedKey();
		if (key != null) {
			generatedKey = apd.addAccountEntryPayment(new AccountEntryPayment(null, line.getId(), table, key));
		}
		return stage;
	}

	public final static Long getGeneratedKey() {
		return generatedKey;
	}
