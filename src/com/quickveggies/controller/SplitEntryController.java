...
public class SplitEntryController implements Initializable {
    ...
    private AccountEntryPaymentDao ad ;
   
    private ExpenditureDao ed ;
    
    private AccountEntryLineDao ald ;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        ad = BeanUtils.getBean(AccountEntryPaymentDao.class);
        ed = BeanUtils.getBean(ExpenditureDao.class);
        ald = BeanUtils.getBean(AccountEntryLineDao.class);
            ...
            public TableCell<AccountEntryLine, Integer> call(TableColumn<AccountEntryLine, Integer> param) {
                ...
                button.setOnMouseClicked((MouseEvent event) -> {
                    ...
                    if (line.getId() != null) {
                        ...
                        ald.setAccountEntryStatus(line.getId(), 0, "Account Entry line removed from InSoftware");
                        AccountEntryPayment payment = ad.getAccountEntryPayment(line.getId());
                        if ("partyMoney".equalsIgnoreCase(payment.getPaymentTable())) {
                        	mpd.deleteMoneyPaidRecd(payment.getPaymentId());
                        }
                        else if ("expenditures".equalsIgnoreCase(payment.getPaymentTable())) {
                            ed.deleteExpenditureEntry(payment.getPaymentId(), true);
                        }
                        ald.deleteAccountEntry(line.getId());
                        ad.deleteAccountPayment(line.getId());
...
//                updateDatabase(entry);
    ...
    private void updateDatabase(AccountEntryLine lineToSave) {
        ...
        ald.saveAccountEntryLine(lineToSave);
        if (remainder <= 0.0) {
        	ald.setAccountEntryStatus(origLine.getId(), -1, "");
        }
    }
